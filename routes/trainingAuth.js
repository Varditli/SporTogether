...
Router.post('/editTraining',requireLoginTrainer,(req,res)=>{
  const {name, capacity,type, time, price, intensity, location, zoom, limitations, gender, age_group, additional_info, created_at, trainerId, trainerUsername} = req.body;
  if(!name || !zoom || !type || !time || !price){
      return res.status(422).json({error:"Please fill all the required fields"})
  }
  Training.findOne({name:name})
  .then((editedTraining)=>{
      ...
      training.save(err => {
        ...
        return res.json({message:"Edited and saved training successfully",training: training})
      })
 })
.catch(err=>{
  console.log(err)
