...
Router.get("/myTrainingsTrainer", requireLoginTrainer, (req, res) => {
  console.log("req.trainer", req.trainer);
  Training.find({ trainerId: req.trainer._id })
...
Router.get("/myTrainingsTrainee", requireLoginTrainee, (req, res) => {
  console.log("myTrainingsTrainee", req.trainee);

  Training.find({ participants: mongoose.Types.ObjectId(req.trainee._id) })
    .then((myTraining) => {
      res.json({ myTraining });
    })
    .catch((err) => {
